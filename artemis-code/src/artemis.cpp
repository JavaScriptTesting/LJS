...
QUrl parseCmd(int argc, char* argv[], artemis::Options& options)
{
    ...
    options.generateFSM = false;
    
    while ((c = getopt_long(argc, argv, "ahsrp:f:t:w:c:i:v:l:m:", long_options, &option_index)) != -1) {
        ...
        case 'i': {
            ...
            options.iterationLimit = 1 * (numOfModel) + (QString(optarg).toInt() - 1) * MAX_TEST_SUIT_SIZE;
            break;
        }
        ...
        case 'm': {
            options.generateFSM = true;
            modelGeneration = true;
            modelIteration = QString(optarg).toInt();
            MAX_TEST_SEQUENCE_LENGTH = modelIteration - 1;
            break;
        }
        case 'l': {
            MAX_TEST_SEQUENCE_LENGTH = QString(optarg).toInt();
            modelIteration = MAX_TEST_SEQUENCE_LENGTH + 1;
            break; 
        }
        case 'w': {
            randomOrWeight = false;
            parameterD = QString(optarg).toDouble();
            break;
        }
...
int main(int argc, char* argv[])
{
    ...
    subjectName = list[list.length() - 2];
